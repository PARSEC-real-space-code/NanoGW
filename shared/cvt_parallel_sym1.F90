...
subroutine cvt_parallel_sym1(gvec, rho, nspin, n_intp, intp)
  ...
  real(dp) :: newpts(3, n_intp), oldpts(3, n_intp), &
              in_mindist(2), out_mindist(2), tsec(2)
  ...
  do iter = 1, max_iter
    ...
    do ig = 1, (igend - igstart + 1)*gvec%syms%ntrans
      igrid = igrid + 1
  ...
  intp = 0
  if (.true.) then
    ! This approach does not check whether there are duplicated pts
    do ipt = istart, iend
    ...
    ! This approach checks whether there are duplicated pts
    select_grid = 0
    do ipt = 1, n_intp
      mindist = 1.e9 ! initialize a very large number
      ! Loop over all the grid points to find a grid point that is closest to newpts(:,ipt)
...
end subroutine cvt_parallel_sym1
